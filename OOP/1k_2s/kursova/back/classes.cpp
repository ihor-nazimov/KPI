...
//CBase
std::ostream& CBase::writeChars(std::ostream& out, const char* s) {
    size_t sz = 0;
    if (s != 0) {
        sz = strlen(s)+1;
...
char* CBase::readChars(std::istream& in) {
    size_t sz = 0;
...
char* CBase::setchars(const char* src, char* dest) {
...
const char* CBase::inputChars(istream& in, ostream& out, const char* prompt, size_t n) {
...
double CBase::inputDouble(istream& in, ostream& out, const char* prompt) {
    double tmp=0;
    do {
        if (!in) {in.clear(); in.ignore(MAX_CHARS, '\n'); } 
        out << prompt;
...
char* CBase::getFilename(char* dest, const char* def_filename, size_t n) {
...
int CBase::getChoice(const char* prompt) {
    int choice=-1;
    do {
...
std::ostream& operator<< (std::ostream &out, CArchive& aentry) {
    out << fixed << right << setw(10) << setprecision(10) << aentry._listsize;
